import { FooterContent, FooterProps } from './footer-types';

function Footer(props: FooterProps) {
    function setFooterContent(contents: FooterContent[]) {
        return  (
            contents.map(content => 
                <div key={content.title}>
                    <h2 className="mb-6 text-sm font-semibold uppercase text-gray-900 dark:text-white">{content.title}</h2>
                    <ul className="font-medium text-gray-500 dark:text-gray-400">
                        {
                            content.contents.map(subContent => 
                                <li key={subContent.title} className="mb-4">
                                    <a href={subContent.link} className="hover:underline">{subContent.title}</a>
                                </li>)
                        }
                    </ul>
                </div>
        ...
        <footer className="mt-40 bg-white dark:bg-gray-900">
            <div className="mx-auto w-full max-w-screen-xl">
                <div className="grid grid-cols-2 gap-8 px-4 py-6 md:grid-cols-4 lg:py-8">
                    {setFooterContent(props.footerContents)}
                </div>
                <div className="bg-gray-100 px-4 py-6 dark:bg-gray-700 md:flex md:items-center md:justify-between">
                    <span className="text-sm text-gray-500 dark:text-gray-300 sm:text-center">© {props.copyrightYear} <a href="https://flowbite.com/">{props.copyrightTitle}</a>. All Rights Reserved.
                    </span>
                    <div className="mt-4 flex space-x-6 sm:justify-center md:mt-0">
                        <a href={encodeURI(props.facebookLink)} className="text-gray-400 hover:text-gray-900 dark:hover:text-white">
                        ...
                        <a href={encodeURI(props.instagramLink)} className="text-gray-400 hover:text-gray-900 dark:hover:text-white">
                        ...
                        <a href={encodeURI(props.twitterLink)} className="text-gray-400 hover:text-gray-900 dark:hover:text-white">
                        ...
                        <a href={encodeURI(props.gitHubLink)} className="text-gray-400 hover:text-gray-900 dark:hover:text-white">
