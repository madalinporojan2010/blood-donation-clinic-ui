...
export default function MessageBox(props: MessageBoxProps) {
    ...
    return (
        <section>
            <div id="alert-1" className={`mb-4 flex rounded-lg border ${'border-' + colors.border_normal} ${'bg-' + colors.bg_normal} p-4 text-base font-bold ${'text-' + colors.text_normal} ${'dark:border-' + colors.border_dark} dark:bg-gray-800 ${'text-' + colors.text_dark}`} role="alert">
                {getSVGIcon(props.messageType)}
                <span className="sr-only">Info</span>
                <div>
                    {props.message}
                </div>
                <button type="button" className={`-m-1 ml-auto inline-flex h-8 w-8 rounded-lg ${'bg-' + colors.bg_normal} p-1.5 ${'text-' + colors.button_text_normal} ${'hover:bg-' + colors.button_hover_normal} focus:ring-2 ${'focus:ring-' + colors.button_focus_normal} dark:bg-gray-800 ${'dark:text-' + colors.button_text_dark} dark:hover:bg-gray-700`} data-dismiss-target="#alert-1" aria-label="Close" onClick={() => {props.onClose();}}>
